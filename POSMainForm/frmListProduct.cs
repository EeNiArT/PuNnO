    ...
    public partial class frmListProduct : Form
    {
        ...
        private void btnUpdate_Click (object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    //this should be call on null reference calling.
                }
                else
                {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            if (ListView1.Items.Count == 0)
            {
                Interaction.MsgBox("Please select record to Delete", MsgBoxStyle.Exclamation, "Delete Products");
                return;
            }
            try
            {
                ...
                else
                {
                    SQLConn.adding = false;
                    SQLConn.updating = true;
                    int pID = Convert.ToInt32(ListView1.FocusedItem.Text);
                    try
                    {
                        SQLConn.sqL = "DELETE FROM 	product WHERE ProductNo = " + pID + "";
                        SQLConn.ConnDB();
                        SQLConn.cmd = new MySqlCommand(SQLConn.sqL, SQLConn.conn);
                        SQLConn.cmd.ExecuteNonQuery();
                        Interaction.MsgBox("Products Deleted.", MsgBoxStyle.Information, "Confirmation Delete Products");
                        LoadProducts("");
                    }
                    catch (Exception ex)
                    {
