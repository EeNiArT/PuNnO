...
namespace POSMainForm
{
    public partial class frmListStaff : Form
    {
        ...
        public void LoadStaffs(string search)
        {
            try
            {
                SQLConn.sqL = "SELECT StaffID,UPassword, CONCAT(Firstname, ' ', Lastname) as ClientName, CONCAT(Street, ', ', City , ', ', Province) as Address, ContactNo, username, role FROM Staff WHERE FIRSTNAME LIKE '" + search.Trim() + "%' ORDER By Firstname";
                SQLConn.ConnDB();
                ...
                while (SQLConn.dr.Read() == true)
                {
                    ...
                    x.SubItems.Add(SQLConn.dr["UPassword"].ToString());
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            SQLConn.strSearch = Interaction.InputBox("ENTER FIRST NAME OF THE STAFF.", "Search Staff", " ");
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            if (ListView1.Items.Count == 0)
            {
                Interaction.MsgBox("Please select record to Delete", MsgBoxStyle.Exclamation, "Delete Staffs");
                return;
            }
            try
            {
                ...
                else
                {
                    SQLConn.adding = false;
                    SQLConn.updating = true;
                    int sID = Convert.ToInt32(ListView1.FocusedItem.Text);
                    try
                    {
                        SQLConn.sqL = "DELETE FROM 	staff WHERE StaffID = " + sID + "";
                        SQLConn.ConnDB();
                        SQLConn.cmd = new MySqlCommand(SQLConn.sqL, SQLConn.conn);
                        SQLConn.cmd.ExecuteNonQuery();
                        Interaction.MsgBox("Staffs Deleted.", MsgBoxStyle.Information, "Confirmation Delete Staffs");
                        LoadStaffs("");
                    }
                    catch (Exception ex)
                    {
